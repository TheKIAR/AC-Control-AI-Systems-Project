"""Classic GUI launcher. Double-click or run: python run_classic.py"""
...
def _apply_classic_icon(window):
    try:
        png = _resource_path("assets", "app_classic.png")
        if png.exists():
        ...
        pass
    try:
        ico = _resource_path("assets", "app_classic.ico")
        if ico.exists():
...
try:
    ...
    from version import CLASSIC_VERSION as APP_VERSION
except ImportError:
    ...
    try:
        ...
        try:
            from src.version import CLASSIC_VERSION as APP_VERSION
        except ImportError:
            APP_VERSION = "1.0.0"
...
class ClassicApp(tk.Tk):
    """Plain white GUI: simple controls on top, charts plus text below."""

    def __init__(self):
        super().__init__()
        self.title(f"AI Systems Project (Classic) v{APP_VERSION}")
        ...
        tk.Label(header, text="AI Systems Project — Classic",
                 bg="#ffffff", fg="#000000",
        ...
        for column in range(3):
            charts.grid_columnconfigure(column, weight=1, uniform="classic")
...
def main():
    ...
    try:
        ...
        print(f"Could not open classic GUI ({type(exc).__name__}: {exc}).")
        print(format_result(compute()))
        return False
